...
// Services
import { stockService } from './services/StockService'; // New Service
import { analyzePortfolio } from './services/geminiService'; // AI only
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToAuthChanges(async (currentUser) => {
      ...
      if (currentUser) {
        ...
        if (cloudData) {
          // Case 1: Cloud data exists -> Use Cloud Data
          console.log("Loaded data from cloud");
          setPortfolio(cloudData);
        } else {
          // Case 2: New user (or first time login) -> Upload local data to cloud (Migration)
          console.log("No cloud data found, uploading local data...");
          await savePortfolioToCloud(currentUser.uid, portfolio);
        }
        setIsSyncing(false);
      }
    });
  ...
  // Save to Cloud (Debounced) or LocalStorage
  useEffect(() => {
  ...
  // --- Price Update Logic (Using StockService) ---
  ...
  // Ref to hold the latest version of the update function to prevent stale closures in setInterval
  ...
  useEffect(() => {
    if (isAutoRefresh) {
      // Execute immediately (using the ref to be safe)
  ...
  const handleRefreshCategory = async (categoryId: string) => {
    ...
    // Updated: Use StockService batch
