...
class StockService {
  ...
  private async fetchWithProxy(targetUrl: string, isJson: boolean = true): Promise<any | null> {
    for (const config of PROXY_CONFIGS) {
      try {
        ...
        console.warn(`Proxy fail: ${config.wrapper}`);
        continue;
      }
    }
    return null;
  ...
  public async getStockNews(symbol: string, market: 'TW' | 'US'): Promise<NewsItem[]> {
    ...
    // 台股加入「股票」關鍵字，美股加「stock」，增加 Google News 辨識度
    const query = market === 'TW' ? `${symbol} 股票` : `${symbol} stock`;
    ...
    try {
      ...
      if (!xmlText || typeof xmlText !== 'string' || xmlText.length < 100) return [];
