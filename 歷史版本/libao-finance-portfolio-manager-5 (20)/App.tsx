...
const App: React.FC = () => {
  ...
  const handleRevokeTransaction = (txId: string) => {
    ...
    const nextCategories = portfolio.categories.map(cat => {
      ...
      if (tx.type === 'BUY') {
        if (assetIdx === -1) return cat; // Should not happen based on logic
        const asset = nextAssets[assetIdx];
        const nextLots = asset.lots.filter(l => l.id !== tx.lotId);
        ...
        // Undo SELL: Put shares back (Simplified: Add a new lot with the original transaction price context)
        ...
        if (assetIdx > -1) {
          ...
          // Re-create asset if it was fully sold
          nextAssets.push({ 
            id: tx.assetId, 
            symbol: tx.symbol, 
            name: tx.name, 
