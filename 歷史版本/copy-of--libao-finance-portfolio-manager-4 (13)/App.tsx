...
const App: React.FC = () => {
  ...
  const tourSteps: TourStep[] = useMemo(() => {
    ...
    if (isMobile) {
      ...
      steps.push(
        {
          targetId: 'tour-news-mobile',
          title: '步驟 3：個股新聞',
          content: '點擊鈴鐺圖示，隨時掌握持倉股票的最新相關新聞。',
        },
        {
          targetId: 'tour-menu-mobile',
          title: '步驟 4：功能選單',
          content: '點擊選單按鈕，可找到「手動更新股價」、「系統設定 (備份)」與「學院資源」等更多功能。',
        }
      );
    } else {
      ...
      steps.push(
        {
          targetId: 'tour-news-desktop',
          title: '步驟 3：個股新聞',
          content: '鈴鐺圖示會顯示您持倉股票的最新相關新聞。如有重大消息，我們也會發送通知提醒您。',
        },
        {
          targetId: 'tour-update-desktop',
          title: '步驟 4：即時股價',
          content: '點擊此按鈕可手動更新所有持倉的最新股價。我們也支援 60 秒自動更新功能。',
        },
        {
          targetId: 'tour-settings-desktop',
          title: '步驟 5：系統設定與備份',
          content: '在此設定美金匯率、手續費計算方式，或是將您的資料匯出備份 (JSON 格式)。',
        }
      );
    }
    return steps;
  ...
  useEffect(() => {
    ...
    if (isOnboarded && localData) {
        try {
            ...
            else parsed.settings.enableSystemNotifications = parsed.settings.enableSystemNotifications ?? false;
  ...
  // ... (Keep other handlers like handleResetPortfolio, handleUpdatePrices, handleImportData, etc. exactly as is)
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-2">
              <button onClick={() => setShowTutorial(true)} className="p-2 rounded-full border border-gray-700 bg-gray-800 text-gray-300 hover:text-white transition-colors mr-2 flex items-center justify-center" title="操作教學">
                <HelpCircle className="w-4 h-4" />
              </button>

              <div className="relative mr-2" id="tour-news-desktop"> {/* Renamed ID for desktop */}
                 <button onClick={() => setShowNewsModal(true)} className={`p-2 rounded-full border border-gray-700 bg-gray-800 text-gray-300 hover:text-white transition-all ${hasNewNews ? 'animate-pulse text-yellow-400' : ''}`} title="持股新聞">
                   <Bell className={`w-4 h-4 ${hasNewNews ? 'fill-yellow-400' : ''}`} />
                 </button>
                 {hasNewNews && <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-gray-900"></span>}
              </div>

              {/* ... (Keep existing User / Auth section) ... */}
              {isFirebaseEnabled && (
              ...
              <button onClick={() => setIsPrivacyMode(!isPrivacyMode)} className={`p-2 rounded-full border transition-all mr-1 ${isPrivacyMode ? 'bg-yellow-100 text-yellow-700 border-yellow-300' : 'bg-gray-800 text-gray-300 border-gray-700 hover:text-white'}`} title={isPrivacyMode ? "顯示金額" : "隱藏金額"}>
                {isPrivacyMode ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>

              <div className="hidden lg:flex items-center bg-gray-800 rounded-full px-3 py-1 border border-gray-700 mr-2">
                 <button onClick={() => setIsAutoRefresh(!isAutoRefresh)} className={`flex items-center gap-2 text-xs font-bold uppercase tracking-wider transition-colors ${isAutoRefresh ? 'text-green-400' : 'text-gray-500'}`}>
                   {isAutoRefresh ? <PauseCircle className="w-4 h-4" /> : <PlayCircle className="w-4 h-4" />}
                   {isAutoRefresh ? 'Auto On (60s)' : 'Auto Off'}
                 </button>
                 {lastUpdated && <span className="ml-3 pl-3 border-l border-gray-600 text-[10px] text-gray-400 flex items-center gap-1"><Clock className="w-3 h-3" />{lastUpdated.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>}
              </div>

               <button 
                id="tour-update-desktop" // Renamed ID for desktop
              ...
              <button 
                id="tour-settings-desktop" // Renamed ID for desktop
            ...
            <div className="md:hidden flex items-center gap-3">
               <div className="relative" id="tour-news-mobile"> {/* Added ID for mobile news */}
                 <button onClick={() => setShowNewsModal(true)} className={`p-2 text-gray-300 hover:text-white ${hasNewNews ? 'text-yellow-400' : ''}`}>
               ...
               {/* ... (Keep existing mobile sync/login) ... */}
               {isFirebaseEnabled && user && (
                  <div className="flex items-center gap-2 bg-gray-800/50 px-2 py-1 rounded-full border border-gray-700/50">
                     <Cloud className="w-4 h-4 text-green-400" />
                     <img src={user.photoURL || `https://ui-avatars.com/api/?name=${user.email}`} className="w-6 h-6 rounded-full border border-gray-500" alt="User"/>
                  </div>
               )}
               {!user && (
                  <button onClick={handleTutorialLogin} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-full font-bold flex items-center gap-1"><LogIn className="w-3 h-3" /> 登入</button>
               )}
               <button 
                 id="tour-menu-mobile" // Added ID for mobile menu
      ...
      {/* Mobile Sidebar */}
      {/* ... (Keep existing Sidebar code but potentially add IDs if needed for mobile tour, though we prioritized desktop/visible IDs) ... */}
      {isSidebarOpen && (
        <div className="fixed inset-0 z-[100] md:hidden">
           ...
           <div className="absolute top-0 right-0 h-full w-[80%] max-w-[300px] bg-gray-900 text-white shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                 ...
                 {/* Academy Resources & Actions ... (Keep existing) */}
                 ...
                 <div className="space-y-2">
                    ...
                    <button onClick={() => { handleUpdatePrices(false); setIsSidebarOpen(false); }} className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-gray-800 active:bg-gray-700 transition-colors group">
                       <div className="flex items-center gap-3"><div className="p-2 bg-gray-800 rounded-full group-hover:bg-gray-700"><RefreshCw className="w-4 h-4 text-blue-400" /></div><span className="text-sm font-medium">手動更新即時股價</span></div><ChevronRight className="w-4 h-4 text-gray-600" />
                    </button>
                    <button onClick={() => { setShowNewsModal(true); setIsSidebarOpen(false); }} className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-gray-800 active:bg-gray-700 transition-colors group">
                       <div className="flex items-center gap-3"><div className="p-2 bg-gray-800 rounded-full group-hover:bg-gray-700"><Bell className="w-4 h-4 text-yellow-400" /></div><span className="text-sm font-medium">持股相關新聞</span></div><ChevronRight className="w-4 h-4 text-gray-600" />
                    </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-8">
              ...
              <div className="md:col-span-8 lg:col-span-9 order-1 md:order-2 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div id="tour-total-capital" className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col justify-between">
