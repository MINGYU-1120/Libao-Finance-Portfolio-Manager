...
const App: React.FC = () => {
  ...
  const handleExecuteOrder = (order: OrderData) => {
    ...
    // 計算此筆交易相對於該倉位預算比例 (加減碼比例)
    const projectedInvestment = Math.floor(portfolio.totalCapital * (currentCategory.allocationPercent / 100));
    
    // 獲取目前資產以便計算 SELL 的比例 (若是 SELL，比例應以扣除的成本 TWD 計算，以保持與總覽一致)
    const assetObj = currentCategory.assets.find(a => a.symbol === order.symbol);
    let orderRatio = 0;

    if (order.action === 'BUY') {
      const amountTWD = order.price * order.shares * order.exchangeRate;
      orderRatio = projectedInvestment > 0 ? (amountTWD / projectedInvestment) * 100 : 0;
    }
    ...
    const nextCategories = portfolio.categories.map(cat => {
      ...
      if (order.action === 'BUY') {
        ...
        newTx.portfolioRatio = orderRatio;
        ...
        newTx.originalCostTWD = totalCostOfSoldSharesTWD; // 記錄賣出部分的歷史原始成本 TWD
        
        // 記錄 SELL 的減碼比例 (以扣除成本佔預算的百分比記錄)
  ...
  /**
   * 修正後的撤銷交易邏輯：確保 SELL 撤銷後比例正確恢復到賣出前
   */
  const handleRevokeTransaction = (txId: string) => {
    ...
    const nextCategories = portfolio.categories.map(cat => {
      ...
      if (tx.type === 'BUY') {
        ...
        // 修正重點：使用紀錄中存下的「originalCostTWD」來計算還原單價
        // 這能保證撤銷後的「總投入成本」完全變回賣出前，進而讓「比例%」精準回復
