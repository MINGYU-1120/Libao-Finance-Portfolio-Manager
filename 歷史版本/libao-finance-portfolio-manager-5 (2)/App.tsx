...
import { PortfolioState, CalculatedCategory, CalculatedAsset, TransactionRecord, AppSettings, NewsItem } from './types';
...
const App: React.FC = () => {
  ...
  // Fix: Added missing state isScanningDividends
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 z-[100]">
           ...
           <div className="absolute top-0 right-0 h-full w-80 bg-gray-900 text-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
              ...
              {/* User Account Section (NEW: AT THE TOP) */}
              <div className="p-6 bg-gray-800/50 border-b border-gray-800">
                 <div className="flex justify-between items-center mb-6">
                    <span className="font-bold text-lg flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-libao-gold"/>雲端資料同步</span>
                        ...
                        <div className="flex items-center gap-3 p-3 bg-gray-900/50 rounded-xl border border-gray-700">
                            ...
                            <div className="text-xs truncate overflow-hidden flex-1">
                               ...
                               <div className="flex items-center gap-1 text-green-400 mt-1 font-bold">
                                  <Cloud className="w-3 h-3" /> 資料已受保護
                               </div>
                            </div>
                        </div>
                        <button onClick={logoutUser} className="w-full py-2.5 bg-red-500/10 text-red-400 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-red-500/20 transition-colors"><LogOut className="w-4 h-4" /> 登出目前帳號</button>
                    </div>
                 ) : (
                    <div className="space-y-4">
                       <p className="text-xs text-gray-400 leading-relaxed">登入 Google 帳號即可將投資資料備份至雲端，更換裝置也不怕資料遺失。</p>
                       <button onClick={loginWithGoogle} className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg transition-transform active:scale-95"><LogIn className="w-4 h-4" /> 立即登入同步</button>
                    </div>
                 )}
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8">
                 <div className="space-y-4">
                    <div className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em] px-1">系統偏好設定</div>
                    <div className="grid grid-cols-1 gap-2">
                       <button onClick={() => { setShowFeeSettingsModal(true); setIsSidebarOpen(false); }} className="w-full flex items-center justify-between p-3.5 rounded-xl bg-gray-800 hover:bg-gray-700 transition-all group">
                          <div className="flex items-center gap-3"><Calculator className="w-5 h-5 text-indigo-400 group-hover:scale-110 transition-transform" /><span className="text-sm font-medium">電子下單折扣設定</span></div>
                       ...
                       <button onClick={() => { setShowSettingsModal(true); setIsSidebarOpen(false); }} className="w-full flex items-center justify-between p-3.5 rounded-xl bg-gray-800 hover:bg-gray-700 transition-all group">
                          <div className="flex items-center gap-3"><Settings className="w-5 h-5 text-gray-400 group-hover:rotate-45 transition-transform" /><span className="text-sm font-medium">匯率 / 備份 / 重置</span></div>
                          <ChevronRight className="w-4 h-4 text-gray-600" />
                       </button>
                    </div>
                 </div>
                 ...
                 <div className="space-y-4">
                    <div className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em] px-1">幫助與導覽</div>
                    <div className="grid grid-cols-1 gap-2">
                       {/* NEW: GUIDED TOUR OPTION */}
                       <button onClick={startManualTour} className="w-full flex items-center justify-between p-3.5 rounded-xl bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/20 transition-all group">
                          <div className="flex items-center gap-3"><HelpCircle className="w-5 h-5 text-blue-400" /><span className="text-sm font-bold text-blue-100">啟動功能教學導覽</span></div>
                       ...
                       <a href="https://line.me/R/ti/p/@607dxbdr" target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-between p-3.5 rounded-xl bg-gray-800 hover:bg-gray-700 transition-all">
                          <div className="flex items-center gap-3"><MessageCircle className="w-5 h-5 text-green-500" /><span className="text-sm font-medium">LINE 官方帳號支援</span></div>
                 ...
                 <div className="space-y-4 pt-4">
                    <div className="p-4 rounded-xl bg-gradient-to-br from-indigo-600 to-blue-700 text-white shadow-lg">
                       ...
                       <p className="text-[10px] text-blue-100 leading-relaxed mb-3">掌握台美股倉位配置，實現資產穩定增長。我們陪伴您走過每一段投資旅程。</p>
                       <a href="https://libao-finance-school.mykajabi.com/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[10px] font-bold bg-white/20 hover:bg-white/30 px-3 py-1.5 rounded-lg transition-colors">前往官網課程 <ChevronRight className="w-3 h-3"/></a>
                    </div>
                 </div>
              </div>
              ...
              <div className="p-6 border-t border-gray-800 text-center">
                 <div className="text-[10px] text-gray-600 font-mono">Libao Portfolio Manager v5.2.0</div>
              </div>
           </div>
        </div>
      )}
                     ...
                     <div className="md:col-span-8 lg:col-span-9 flex flex-col gap-6">
                        <div id="tour-total-capital" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                               <div className="text-gray-400 text-xs font-bold mb-1">總資產淨值</div>
                               <div className="text-xl font-mono font-bold">NT$ {maskValue(Math.round(portfolio.totalCapital + calculatedData.totalRealizedPnL + calculatedData.totalUnrealizedPnL).toLocaleString())}</div>
                            </div>
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                               <div className="text-gray-400 text-xs font-bold mb-1">投入本金</div>
                               <div className="text-xl font-mono font-bold flex justify-between items-center">
                                   <span>NT$ {maskValue(portfolio.totalCapital.toLocaleString())}</span>
                                   <button onClick={() => setShowCapitalModal(true)} className="text-[10px] bg-gray-100 px-2 py-1 rounded text-blue-600 font-bold hover:bg-blue-50 border border-gray-200">出/入金</button>
                               </div>
                            </div>
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                               <div className="text-gray-400 text-xs font-bold mb-1">未實現損益</div>
                               <div className={`text-xl font-mono font-bold ${calculatedData.totalUnrealizedPnL >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                                   {calculatedData.totalUnrealizedPnL > 0 ? '+' : ''}{maskValue(calculatedData.totalUnrealizedPnL.toLocaleString())}
                               </div>
                            </div>
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                               <div className="text-gray-400 text-xs font-bold mb-1">已實現損益</div>
                               <div className={`text-xl font-mono font-bold ${calculatedData.totalRealizedPnL >= 0 ? 'text-red-600' : 'text-green-600'}`}>
