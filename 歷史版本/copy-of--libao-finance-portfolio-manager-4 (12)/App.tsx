...
const App: React.FC = () => {
  ...
  const fetchStockNews = useCallback(async () => {
    ...
    try {
      ...
      // Fetch in parallel but limit concurrency if needed. 
      // For RSS via proxy, parallel is usually fine for < 20 items.
      ...
      // Filter valid dates and Sort desc
      ...
      // Only set notification if there is news AND it is newer than what we last saw
      if (sorted.length > 0) {
         ...
         if (!lastReadTimestamp || latestNewsTimestamp > parseInt(lastReadTimestamp, 10)) {
             ...
             if (portfolioRef.current.settings.enableSystemNotifications && 
                 ...
                 const notificationTitle = `Libao 發現 ${sorted.length} 則相關新聞`;
  ...
  useEffect(() => {
    ...
    if (isOnboarded && localData) {
        try {
            ...
            // Ensure settings structure exists even if old data format
            if (!parsed.settings) {
                parsed.settings = initialPortfolioState.settings;
            } else {
                // Ensure notification setting is preserved or defaulted
            ...
            // Trigger background news fetch shortly after load
  ...
  const handleTutorialLogin = async () => {
     ...
     if (!loggedInUser) {
        // User cancelled login -> Stay on empty dashboard (Guest Mode)
     }
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (cloudData) {
             ...
             // Robust check for notification setting during sync
  ...
  const handleLogout = async () => {
     ...
     try {
         ...
         // CHANGE: Show Tutorial (Welcome Screen) instead of Onboarding
         setShowOnboarding(false);
         setShowTutorial(true); 
  ...
  const handleResetPortfolio = async () => {
    if (!window.confirm("⚠️ 危險操作警告\n\n此操作將「永久刪除」所有交易紀錄、設定與資產資料，且無法復原！\n\n您確定要重置整個帳戶並重新初始化嗎？")) return;
    
    if (!window.confirm("再次確認：您真的要刪除所有資料嗎？(資料將完全消失)")) return;
    ...
    try {
        ...
        localStorage.removeItem('libao-news-last-read'); // Clear news read status on reset
        
        showToast("系統已重置", 'success');
        
        // Reload to trigger clean init flow
  ...
  const handleUpdatePrices = async (isSilent = false) => {
    ...
    try {
      ...
      // Also refresh news when manually updating prices
    ...
    <div className="min-h-screen bg-gray-100 pb-20">
      ...
      <NewsModal 
        ...
        onClose={() => {
           ...
           setHasNewNews(false); // Clear notification badge on close
           // BUG FIX: Ensure we pick the LATEST timestamp from all news items
           if (newsItems.length > 0) {
               const timestamps = newsItems
                  .map(n => new Date(n.pubDate).getTime())
                  .filter(t => !isNaN(t) && t > 0);
               
               if (timestamps.length > 0) {
                   const latestTime = Math.max(...timestamps);
                   localStorage.setItem('libao-news-last-read', latestTime.toString());
               }
           }
        }}
        newsItems={newsItems}
      ...
      <nav className="bg-gray-900 text-white shadow-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
              ...
              <div className="relative group ml-1">
                 ...
                 <div className="absolute top-full right-0 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[60]">
                    <div className="bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden w-48 py-1">
                       <a 
                         href="https://line.me/R/ti/p/@607dxbdr"
                         target="_blank"
                         rel="noopener noreferrer"
                         className="px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-green-600 flex items-center gap-2 transition-colors"
                       >
                          <MessageCircle className="w-4 h-4 text-green-500"/> LINE 官方帳號
                       </a>
                       <a 
                         href="https://libao-finance-school.mykajabi.com/products/6/categories"
                         target="_blank"
                         rel="noopener noreferrer"
                         className="px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-blue-600 flex items-center gap-2 border-t border-gray-100 transition-colors"
                       >
                          <BookOpen className="w-4 h-4 text-blue-500"/> 投資組合參考
                       </a>
                    </div>
                 </div>
              </div>
            ...
            {/* Desktop Right Toolbar (Hidden on Mobile) */}
            <div className="hidden md:flex items-center gap-2">
              ...
              <div className="relative mr-2">
                 <button 
                   ...
                   className={`p-2 rounded-full border border-gray-700 bg-gray-800 text-gray-300 hover:text-white transition-all ${hasNewNews ? 'animate-pulse text-yellow-400' : ''}`}
                   title="持股新聞"
                 >
                   <Bell className={`w-4 h-4 ${hasNewNews ? 'fill-yellow-400' : ''}`} />
                 </button>
                 {hasNewNews && (
              ...
              <div className="hidden lg:flex items-center bg-gray-800 rounded-full px-3 py-1 border border-gray-700 mr-2">
                 <button 
            ...
            <div className="md:hidden flex items-center gap-3">
               ...
               <div className="relative">
                 <button 
                   onClick={() => setShowNewsModal(true)}
                   className={`p-2 text-gray-300 hover:text-white ${hasNewNews ? 'text-yellow-400' : ''}`}
                 >
                   <Bell className={`w-6 h-6 ${hasNewNews ? 'fill-yellow-400' : ''}`} />
                 </button>
                 {hasNewNews && (
               ...
               {isFirebaseEnabled && user && (
                  <div className="flex items-center gap-2 bg-gray-800/50 px-2 py-1 rounded-full border border-gray-700/50">
               ...
               {!user && (
                  <button onClick={handleTutorialLogin} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-full font-bold flex items-center gap-1">
                     <LogIn className="w-3 h-3" /> 登入
                  </button>
               )}

               {/* Hamburger Menu Trigger */}
               <button onClick={() => setIsSidebarOpen(true)} className="p-2 text-gray-300 hover:text-white active:scale-95 transition-transform">
           ...
           <div className="absolute top-0 right-0 h-full w-[80%] max-w-[300px] bg-gray-900 text-white shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                 ...
                 <div className="space-y-1">
                    ...
                    <button 
                      onClick={() => setIsPrivacyMode(!isPrivacyMode)}
                      className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-gray-800 transition-colors bg-gray-800/40 border border-gray-800"
                    >
                 ...
                 {/* Academy Resources Group (Moved Up) */}
                 <div className="space-y-2">
                    ...
                    <a 
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-gray-800 active:bg-gray-700 transition-colors group"
                    >
                       <div className="flex items-center gap-3">
                          <div className="p-2 bg-gray-800 rounded-full group-hover:bg-gray-700"><MessageCircle className="w-4 h-4 text-green-400" /></div>
                          <span className="text-sm font-medium">LINE 官方帳號</span>
                       </div>
                    ...
                    >
                       <div className="flex items-center gap-3">
                          <div className="p-2 bg-gray-800 rounded-full group-hover:bg-gray-700"><BookOpen className="w-4 h-4 text-blue-400" /></div>
                          <span className="text-sm font-medium">投資組合參考 (會員限定)</span>
                       </div>
                       <ChevronRight className="w-4 h-4 text-gray-600" />
                    </a>
                 </div>

                 {/* Actions Group (Moved Down) */}
                 <div className="space-y-2">
                    <div className="text-xs font-bold text-gray-500 uppercase tracking-wider px-1 mb-1">系統操作</div>
                    
                    <button 
                      onClick={() => { handleUpdatePrices(false); setIsSidebarOpen(false); }}
                      className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-gray-800 active:bg-gray-700 transition-colors group"
                    >
                       <div className="flex items-center gap-3">
                          <div className="p-2 bg-gray-800 rounded-full group-hover:bg-gray-700"><RefreshCw className="w-4 h-4 text-blue-400" /></div>
                          <span className="text-sm font-medium">手動更新即時股價</span>
                       </div>
                       <ChevronRight className="w-4 h-4 text-gray-600" />
                    </button>

                    <button 
                      onClick={() => { setShowNewsModal(true); setIsSidebarOpen(false); }}
                      className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-gray-800 active:bg-gray-700 transition-colors group"
                    >
                       <div className="flex items-center gap-3">
                          <div className="p-2 bg-gray-800 rounded-full group-hover:bg-gray-700"><Bell className="w-4 h-4 text-yellow-400" /></div>
                          <span className="text-sm font-medium">持股相關新聞</span>
                       </div>
                       <ChevronRight className="w-4 h-4 text-gray-600" />
                    </button>
                    ...
                    >
                       <div className="flex items-center gap-3">
                          <div className="p-2 bg-gray-800 rounded-full group-hover:bg-gray-700"><Settings className="w-4 h-4 text-gray-300" /></div>
                          <span className="text-sm font-medium">系統設定 / 備份 / 重置</span>
                       </div>
                       <ChevronRight className="w-4 h-4 text-gray-600" />
                    </button>

                    <button 
                      onClick={() => { setShowTutorial(true); setIsSidebarOpen(false); }}
                      className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-gray-800 active:bg-gray-700 transition-colors group"
                    >
                       <div className="flex items-center gap-3">
                          <div className="p-2 bg-gray-800 rounded-full group-hover:bg-gray-700"><HelpCircle className="w-4 h-4 text-yellow-400" /></div>
                          <span className="text-sm font-medium">操作教學</span>
                       </div>
                       <ChevronRight className="w-4 h-4 text-gray-600" />
                    </button>
                 </div>
              ...
              <div className="p-4 text-center text-[10px] text-gray-600 border-t border-gray-800 bg-black/20">
                 v4.0.0 Libao Finance Academy
              </div>
           </div>
        </div>
      )}
