...
const App: React.FC = () => {
  ...
  // Special state to force order modal open during tour
  ...
  const tourSteps: TourStep[] = useMemo(() => {
    // Determine nav target IDs based on device (Mobile has bottom tab, Desktop has top nav)
    ...
    // Note: Use a dummy ID or skip steps if element doesn't exist on device
    const desktopOnlySteps: TourStep[] = isMobile ? [] : [
       { targetId: 'nav-desktop-academy', title: '學院資源', content: '這裡有連結可前往 LINE 官方帳號與投資組合參考資源。' },
       { targetId: 'nav-desktop-sync', title: '資料同步', content: '顯示您的 Google 登入狀態。綠色雲朵代表資料已安全備份至雲端。' }
    ];
    ...
    const steps: TourStep[] = [
      ...
      { targetId: dividendId, title: '功能：股息帳本', content: '專門管理您的現金股利，並提供一鍵掃描功能自動補登。' },
      ...desktopOnlySteps,
      { targetId: 'tour-news-btn', title: '持股新聞', content: '點擊鈴鐺，自動搜尋您庫存股票的相關新聞。' },
      { targetId: 'tour-privacy-btn', title: '隱私模式', content: '在公共場合想查看持倉？點擊眼睛圖示可隱藏敏感金額。' },
      { targetId: 'tour-update-btn', title: '手動更新', content: '點擊此處立即更新最新股價。我們也有提供 60 秒自動更新功能。' },
      { targetId: 'tour-menu-btn', title: '更多選單', content: '包含系統設定、匯出/匯入備份、重置帳戶與教學導覽。' },
      
      // 2. Dashboard Content
      { targetId: 'tour-total-capital', title: '資金戰情室', content: '這裡監控您的總本金、已投入金額與即時損益狀況。點擊「出/入金」可調整本金。' },
      { targetId: isMobile ? 'tour-summary-mobile-first' : 'tour-summary-table', title: '持倉配置表', content: 'Libao 核心！這裡顯示各個策略倉位 (如G倉、D倉) 的資金佔比與水位。' },
      
      // 3. Drill Down (Enter Category)
      { targetId: 'tour-summary-table', title: '進入倉位明細', content: '我們現在模擬點擊進入第一個倉位，查看詳細持股。' },
      { targetId: 'detail-header', title: '倉位儀表板', content: '上方顯示此倉位的目前資金的使用與損益概況。' },
      { targetId: 'detail-add-btn', title: '新增交易', content: '點擊「下單」按鈕來記錄買入、賣出。' },
      
      // 4. Order Modal
      { targetId: 'order-modal-content', title: '智慧下單介面', content: '輸入代號、金額、投入比例，系統會自動計算預估扣款、手續費與稅金，就算數學不好也不用擔心。' },
    ];
    return steps;
  }, [isMobile]);

  // --- Tour Step Logic Handler ---
  const handleTourStepChange = (stepIndex: number) => {
     // Indices logic based on the `tourSteps` array construction above
     // Basic Navigation Steps: 0 ~ (6 or 8 depending on mobile)
     // Dashboard Steps: +2
     // Drill Down Steps: +3
     // Order Modal: +1
     
     // Calculate indices dynamically because desktopOnlySteps length varies
     const navStepsCount = isMobile ? 7 : 9;
     const dashboardStepsCount = 2;
     const enterCategoryIndex = navStepsCount + dashboardStepsCount; // The step "進入倉位明細"
     const addBtnIndex = enterCategoryIndex + 2; // The step "新增交易"
     const openModalIndex = addBtnIndex + 1; // The step "智慧下單介面"

     // Reset states if going back
     if (stepIndex < enterCategoryIndex) {
        if (activeCategoryId !== null) setActiveCategoryId(null);
        if (isTourForceOrderOpen) setIsTourForceOrderOpen(false);
        setViewMode('PORTFOLIO');
     }

     // 1. Enter Category Logic
     if (stepIndex === enterCategoryIndex) {
        // Find first category to enter
        const firstCatId = portfolioRef.current.categories[0]?.id;
        if (firstCatId && activeCategoryId !== firstCatId) {
           setActiveCategoryId(firstCatId);
        }
     }

     // 2. Open Order Modal Logic
     if (stepIndex === openModalIndex) {
        if (!isTourForceOrderOpen) setIsTourForceOrderOpen(true);
     } else {
        // Close modal if moving away from that step (next or prev)
        if (isTourForceOrderOpen) setIsTourForceOrderOpen(false);
     }
  };
  ...
  const handleTourComplete = () => {
     ...
     setActiveCategoryId(null); // Return to overview
     setViewMode('PORTFOLIO');
  };
  ...
  const handleTutorialClose = () => {
     setShowTutorial(false);
     localStorage.setItem('libao-tutorial-seen-v5', 'true');
     // FORCE SWITCH VIEW to Portfolio to ensure tour elements exist
     setViewMode('PORTFOLIO');
  ...
  // Fix: This handles automatic background saving whenever portfolio state changes
  useEffect(() => {
    if (user && isDataLoaded && isFirebaseEnabled) {
      const saveTimer = setTimeout(() => {
        savePortfolioToCloud(user.uid, portfolio);
      }, 500); // 500ms Debounce
      return () => clearTimeout(saveTimer);
    }
  }, [portfolio, user, isDataLoaded, isFirebaseEnabled]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden lg:flex items-center bg-gray-800 rounded-lg p-1 mx-4">
              <button id="nav-desktop-dashboard" onClick={() => { setActiveCategoryId(null); setViewMode('PORTFOLIO'); }} className={`px-4 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 transition-all ${viewMode === 'PORTFOLIO' ? 'bg-gray-700 text-white shadow-sm' : 'text-gray-400 hover:text-white'}`}>
                <LayoutDashboard className="w-4 h-4" /> 持倉總覽
              </button>
              <button id="nav-desktop-history" onClick={() => { setActiveCategoryId(null); setViewMode('HISTORY'); }} className={`px-4 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 transition-all ${viewMode === 'HISTORY' ? 'bg-gray-700 text-white shadow-sm' : 'text-gray-400 hover:text-white'}`}>
                <History className="w-4 h-4" /> 交易紀錄
              </button>
              <button id="nav-desktop-dividend" onClick={() => { setActiveCategoryId(null); setViewMode('DIVIDENDS'); }} className={`px-4 py-1.5 rounded-md text-sm font-medium flex items-center gap-2 transition-all ${viewMode === 'DIVIDENDS' ? 'bg-purple-700 text-white shadow-sm' : 'text-gray-400 hover:text-white'}`}>
            ...
            <div className="flex items-center gap-1 md:gap-2">
               ...
               {isFirebaseEnabled && (
                <>
                  ...
                  <div id="nav-desktop-sync" className="hidden md:flex items-center mr-1 border-r border-gray-700 pr-3">
                    {user ? (
                  ...
                  <div className="md:hidden flex items-center">
                    {user ? (
                      <button onClick={handleLogout} className="p-2 text-green-400" title="已同步 (點擊登出)">
                         <Cloud className="w-5 h-5" />
                      </button>
                    ) : (
                      <button onClick={handleTutorialLogin} className="p-2 text-gray-300" title="點擊登入">
               ...
               {/* Privacy Toggle */}
               <button id="tour-privacy-btn" onClick={() => setIsPrivacyMode(!isPrivacyMode)} className={`p-2 rounded-full hover:bg-gray-800 transition-all ${isPrivacyMode ? 'text-yellow-400' : 'text-gray-300 hover:text-white'}`} title={isPrivacyMode ? "顯示金額" : "隱藏金額"}>
          ...
          <div className="md:hidden flex border-t border-gray-800 mt-2">
            <button id="nav-mobile-dashboard" onClick={() => { setActiveCategoryId(null); setViewMode('PORTFOLIO'); }} className={`flex-1 py-3 text-sm font-medium flex justify-center items-center gap-2 ${viewMode === 'PORTFOLIO' ? 'text-libao-gold border-b-2 border-libao-gold' : 'text-gray-400'}`}><LayoutDashboard className="w-4 h-4" /> 持倉</button>
            <button id="nav-mobile-history" onClick={() => { setActiveCategoryId(null); setViewMode('HISTORY'); }} className={`flex-1 py-3 text-sm font-medium flex justify-center items-center gap-2 ${viewMode === 'HISTORY' ? 'text-libao-gold border-b-2 border-libao-gold' : 'text-gray-400'}`}><History className="w-4 h-4" /> 紀錄</button>
            <button id="nav-mobile-dividend" onClick={() => { setActiveCategoryId(null); setViewMode('DIVIDENDS'); }} className={`flex-1 py-3 text-sm font-medium flex justify-center items-center gap-2 ${viewMode === 'DIVIDENDS' ? 'text-purple-400 border-b-2 border-purple-400' : 'text-gray-400'}`}><Coins className="w-4 h-4" /> 股息</button>
          </div>
        </div>
      </nav>
            ...
            <div id="tour-summary-table">
                {activeCategoryId && activeCategory ? (
                  <DetailTable 
                    ...
                    isPrivacyMode={isPrivacyMode}
                    settings={portfolio.settings}
                    forceShowOrderModal={isTourForceOrderOpen} // New Prop for Tour
                  />
                ) : (
                  <SummaryTable 
