...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // CHANGE: Removed the Priority 3 block that auto-showed Onboarding for guests.
    // Guests will land on empty dashboard and can choose to Login (triggering sync/onboarding) 
    // or use manually.
  }, []);
  ...
  const handleTutorialLogin = async () => {
     ...
     if (!loggedInUser) {
        ...
        // Removed forced Onboarding here too to respect "Logout shouldn't jump to this page" logic
        // which implies guest state shouldn't be aggressive.
     }
  ...
  const handleTutorialClose = () => {
     ...
     // CHANGE: Removed auto-trigger of Onboarding here.
     // User lands on dashboard. Can login from header if they want.
  };
  ...
  const handleResetPortfolio = async () => {
    ...
    try {
        ...
        localStorage.removeItem('libao-portfolio');
        localStorage.removeItem('libao-onboarding-v1');
        // CHANGE: Also clear tutorial flag to ensure full fresh start
        localStorage.removeItem('libao-tutorial-seen-v5'); 
        
        showToast("系統已重置", 'success');
        
        // Reload to trigger clean init flow (This satisfies "2. 手動重置後出現")
