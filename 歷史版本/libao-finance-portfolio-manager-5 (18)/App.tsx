...
const App: React.FC = () => {
  ...
  const handleLoginAction = async () => {
    ...
    if (loggedInUser) {
      // Data will be loaded via auth change listener
    } else {
      setIsSyncing(false);
    }
  };
  ...
  const fetchStockNews = useCallback(async () => {
    ...
    try {
      const promises = assetsList.map(asset => stockService.getStockNews(asset.symbol, asset.market, asset.name));
  ...
  const handleTourStepChange = (stepIndex: number) => {
    ...
    // Logic to auto-navigate during tour
    ...
    // 1. If we are entering Detail related steps, open the first category
    if (step.title === '進入倉位明細' || isDetailStep) {
       ...
       // If we move back to Summary steps, close detail
       if (activeCategoryId !== null) setActiveCategoryId(null);
    }

    // 2. If we reach Order Modal step, force open it
    if (isOrderStep) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-20">
      ...
      <OnboardingModal isOpen={showOnboarding} initialCategories={DEFAULT_CATEGORIES} onComplete={(cap, cats) => { saveAndSetPortfolio({ ...portfolio, totalCapital: cap, categories: cats }); setShowOnboarding(false); localStorage.setItem('libao-onboarding-v1', 'true'); setTimeout(() => setIsTourActive(true), 1000); }} />
      {showTutorial && (
            ...
            <div className="flex items-center gap-1 md:gap-2">
               ...
               <div id="nav-desktop-sync" className="hidden md:flex items-center border-l border-gray-700 pl-3 ml-1">
                  {user ? (
                    ...
                    <button onClick={handleLoginAction} className="text-gray-400 hover:text-white text-xs font-bold flex items-center gap-1"><Cloud className="w-4 h-4" />未登入</button>
                  )}
               </div>
        ...
        <div className="fixed inset-0 z-[100]">
           ...
           <div className="absolute top-0 right-0 h-full w-80 bg-gray-900 text-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-gray-900">
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                       ...
                       <button onClick={() => { setShowTutorial(true); setIsSidebarOpen(false); }} className="w-full flex items-center justify-between p-3.5 rounded-xl bg-gray-800 hover:bg-gray-700 transition-all group">
                          <div className="flex items-center gap-3"><ShieldAlert className="w-5 h-5 text-gray-400" /><span className="text-sm font-medium">免責聲明</span></div>
                          <ChevronRight className="w-4 h-4 text-gray-600" />
                       </button>
                    </div>
                 </div>
                 ...
                 <div className="pt-4">
                    <div className="p-4 rounded-xl bg-gradient-to-br from-indigo-600 to-blue-700 text-white shadow-lg">
                       ...
                       <p className="text-[10px] text-blue-100 leading-relaxed mb-3">專注於長期價值培養，建立金融市場與資產配置的完整認知。</p>
              ...
              <div className="p-6 border-t border-gray-800 text-center bg-gray-900">
                 <div className="text-[10px] text-gray-600 font-mono">Libao Portfolio Manager v5.6.0</div>
              </div>
           </div>
        </div>
      )}
