...
class StockService {
  ...
  public async getStockNews(symbol: string, market: 'TW' | 'US'): Promise<NewsItem[]> {
    // Google News RSS URL
    // TW: https://news.google.com/rss/search?q={SYMBOL}+stock&hl=zh-TW&gl=TW&ceid=TW:zh-Hant
    // US: https://news.google.com/rss/search?q={SYMBOL}+stock&hl=en-US&gl=US&ceid=US:en
    
    const region = market === 'TW' ? 'TW' : 'US';
    const lang = market === 'TW' ? 'zh-TW' : 'en-US';
    const ceid = market === 'TW' ? 'TW:zh-Hant' : 'US:en';
    const suffix = market === 'TW' ? '股' : 'stock';
    ...
    try {
      ...
      const news: NewsItem[] = [];
      // Limit to top 3 per stock to avoid clutter
