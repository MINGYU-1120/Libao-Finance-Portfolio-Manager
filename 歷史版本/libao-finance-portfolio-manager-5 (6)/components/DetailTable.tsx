    ...
    <div className="bg-white shadow-xl rounded-lg overflow-hidden border border-gray-200 animate-in fade-in zoom-in duration-300 w-full relative">
      ...
      <div id="detail-header" className={`px-4 py-5 text-white flex flex-wrap justify-between items-center gap-4 ${category.market === 'TW' ? 'bg-red-700' : 'bg-blue-800'}`}>
        ...
        <div className="flex gap-6 text-right w-full sm:w-auto justify-between sm:justify-end border-t sm:border-none border-white/10 pt-3 sm:pt-0">
          <div>
            ...
            <div className="text-xl font-mono font-bold leading-none mt-1">{maskValue((category.investedAmount/10000).toFixed(1))}萬 <span className="text-xs opacity-60">({category.investmentRatio.toFixed(1)}%)</span></div>
          </div>
          <div>
      ...
      <div className="sticky top-[64px] z-40 bg-white border-b border-gray-200 px-3 py-2 flex justify-between items-center shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => handleOpenOrder()} className="bg-libao-gold hover:bg-yellow-400 text-gray-900 font-black py-1.5 px-6 rounded-lg text-xs flex items-center gap-1.5 shadow transition-all active:scale-95 border-b-2 border-yellow-600">
                ...
                return (
                  <div key={asset.id} className="bg-white rounded-xl p-3 shadow-sm border border-gray-100 cursor-pointer" onClick={() => handleOpenOrder(asset)}>
                    ...
                    <div className="flex justify-between items-center bg-gray-50 p-2 rounded-lg">
                      <div>
                          <div className="text-[8px] text-gray-400 uppercase">持有 / 占比</div>
                          <div className="font-mono font-bold text-[10px]">{maskValue(asset.shares.toLocaleString())}股 | {asset.portfolioRatio.toFixed(1)}%</div>
                      </div>
                      <div className="text-right">
          ...
          <div className="w-full overflow-x-auto bg-white border-t border-gray-100">
            <table className="w-full text-[12px] text-left border-collapse table-fixed">
              <thead className="bg-gray-50 border-b border-gray-200 font-black text-gray-500 uppercase">
                <tr>
                  <th className="px-1.5 py-2 text-center w-[9%]">代號</th>
                  <th className="px-1.5 py-2 w-[18%]">公司名稱</th>
                  <th className="px-1.5 py-2 text-center w-[11%]">資金占比</th>
                  <th className="px-1.5 py-2 text-right bg-yellow-50/40 w-[11%]">現有股數</th>
                  <th className="px-1.5 py-2 text-right bg-yellow-50/40 w-[11%]">成本均價</th>
                  <th className="px-1.5 py-2 text-right w-[11%]">持倉市值</th>
                  <th className="px-1.5 py-2 text-right bg-red-50/20 w-[11%]">預估損益</th>
                  <th className="px-1.5 py-2 text-center w-[8%]">損益率</th>
                  <th className="px-1.5 py-2 text-center w-[10%]">工具</th>
                </tr>
              </thead>
                ...
                {assets.map((asset) => {
                  ...
                  return (
                    <tr key={asset.id} className="hover:bg-blue-50/20 transition-colors bg-white">
                      ...
                      <td className="px-1.5 py-1.5 font-bold text-gray-800 truncate">{asset.name}</td>
                      <td className="px-1.5 py-1.5 text-center">
                      ...
                      <td className="px-1.5 py-1.5 text-right font-mono font-bold text-gray-700">{maskValue(asset.shares.toLocaleString())}</td>
                      <td className="px-1.5 py-1.5 text-right font-mono text-gray-400">
                        {currencyPrefix}{maskValue(asset.avgCost.toLocaleString(undefined, { minimumFractionDigits: 1 }))}
                      </td>
                      <td className="px-1.5 py-1.5 text-right font-mono font-bold text-gray-800">
                        {maskValue(Math.round(displayMarketValue).toLocaleString())}
                      </td>
                      <td className={`px-1.5 py-1.5 text-right font-mono font-bold ${asset.unrealizedPnL >= 0 ? 'text-red-500' : 'text-green-600'}`}>
