...
export const calculateIndustryAnalysis = (categories: any[], usExchangeRate: number) => {
  const map: Record<string, number> = {};
  let totalValue = 0;

  categories.forEach(cat => {
    const rate = cat.market === 'US' ? usExchangeRate : 1;
    cat.assets.forEach((asset: any) => {
      ...
      map[industry] = (map[industry] || 0) + marketValue;
