...
const App: React.FC = () => {
  ...
  const [portfolio, setPortfolio] = useState<PortfolioState>(() => {
    ...
    if (saved) {
      try {
        ...
        if (!parsed.lastModified) parsed.lastModified = Date.now();
        return parsed;
      } catch (e) {
  ...
  useEffect(() => {
    const hasSeenTutorial = localStorage.getItem('libao-tutorial-seen-v3');
    if (!hasSeenTutorial) {
  ...
  // Writes to localStorage IMMEDIATELY to prevent data loss on quick close
  const saveAndSetPortfolio = useCallback((newState: PortfolioState) => {
      ...
      try {
          ...
          showToast("警告：瀏覽器儲存空間已滿或失敗，資料可能未保存。", "error");
      }
      
      // 2. State Update
      setPortfolio(newState);
      
      // 3. Trigger Cloud Sync (will be debounced by useEffect)
  }, [showToast]);
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (cloudData) {
            ...
            await savePortfolioToCloud(currentUser.uid, portfolioRef.current);
            showToast("已建立雲端備份", 'success');
          }
        } catch (e) {
          console.error("Sync failed", e);
          showToast("雲端同步發生錯誤，請檢查網路", 'error');
        } finally {
          setIsSyncing(false);
        }
      }
  ...
  const handleLogout = async () => {
     if(!window.confirm("登出將會清除此裝置上的所有資料，請確認您已同步備份。確定要登出嗎？")) return;
     
     await logoutUser();
     console.log("User logged out. Resetting portfolio.");
  ...
  useEffect(() => {
    // Redundant safety save, though saveAndSetPortfolio handles primary persistence
    try {
    ...
    if (user && isFirebaseEnabled) {
      // Reduced debounce time to 500ms to catch quick closes better
  ...
  const handleUpdatePrices = async (isSilent = false) => {
    ...
    try {
      ...
      if (!isSilent) showToast("股價更新失敗，請稍後再試", 'error');
    } finally {
  ...
  const handleScanDividends = async () => {
    ...
    try {
        ...
        portfolioRef.current.categories.forEach(c => {
           c.assets.forEach(a => {
               // Use symbol + market as unique key
