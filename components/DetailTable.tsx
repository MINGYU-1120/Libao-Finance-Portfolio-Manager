    ...
    <div className="bg-white shadow-xl rounded-lg overflow-hidden border border-gray-200 animate-in fade-in zoom-in duration-300">
      ...
      <div className="p-3 bg-gray-50 border-b flex justify-between items-center sticky top-0 z-10">
        <span className="text-sm text-gray-500">
            {assets.length === 0 ? '請開始下單' : `共 ${assets.length} 檔`}
        </span>
        <button
            ...
            <div key={asset.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-200" onClick={() => handleOpenOrder(asset)}>
               ...
               <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="text-right flex items-center gap-2">
                     <span className="font-mono font-bold text-gray-800 bg-yellow-50 px-2 py-0.5 rounded border border-yellow-100">
                        {maskValue(asset.currentPrice.toLocaleString())}
                     </span>
                     <button 
               ...
               <div className="grid grid-cols-2 gap-3 mb-3 text-sm">
                  ...
                  <div className="bg-gray-50 p-2 rounded text-right">
                     <div className="text-xs text-gray-400">持有成本</div>
                     <div className="font-mono font-medium">{maskValue(asset.costBasis.toLocaleString())}</div>
                  </div>
                  <div className="bg-gray-50 p-2 rounded">
                     <div className="text-xs text-gray-400">市值</div>
                     <div className="font-mono font-bold text-gray-800">{maskValue(asset.marketValue.toLocaleString())}</div>
                  </div>
                  <div className={`p-2 rounded text-right ${asset.unrealizedPnL >= 0 ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
                     ...
                     <div className="font-mono font-bold flex flex-col">
                        <span>{isPrivacyMode ? '****' : (asset.unrealizedPnL > 0 ? '+' : '') + asset.unrealizedPnL.toLocaleString()}</span>
        ...
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-gray-700 uppercase bg-gray-100 border-b">
            <tr>
              ...
              <th className="px-3 py-3 text-right">投資成本 (TWD)</th>
              ...
              <th className="px-3 py-3 text-right">持倉價值 (TWD)</th>
              <th className="px-3 py-3 text-right">未實現損益</th>
              <th className="px-3 py-3 text-center">損益率</th>
              <th className="px-3 py-3 text-right text-gray-500">已實現</th>
            ...
            {assets.map((asset) => (
              <tr key={asset.id} className="border-b hover:bg-gray-50 group">
                ...
                <td className="px-3 py-3 text-right text-gray-600 font-mono">
                  {maskValue(asset.avgCost.toLocaleString())}
                  {category.market === 'US' && !isPrivacyMode && <span className="text-[10px] text-gray-400 block">USD</span>}
                </td>
                <td className="px-3 py-3 text-right font-mono text-gray-400">{maskValue(asset.costBasis.toLocaleString())}</td>
                ...
                <td className="px-3 py-3 text-right font-mono font-medium">{maskValue(asset.marketValue.toLocaleString())}</td>
                <td className={`px-3 py-3 text-right font-mono ${asset.unrealizedPnL >= 0 ? 'text-red-500' : 'text-green-600'}`}>
                  {maskValue(asset.unrealizedPnL.toLocaleString())}
                </td>
                ...
                <td className={`px-3 py-3 text-right font-mono text-xs ${asset.realizedPnL >= 0 ? 'text-red-500' : 'text-green-600'}`}>
                   {asset.realizedPnL !== 0 ? maskValue(asset.realizedPnL.toLocaleString()) : '-'}
                </td>
