...
const StrategyPerformanceChart: React.FC<StrategyPerformanceChartProps> = ({ picks }) => {
    ...
    const hasBaseData = picks.filter(p => p.status === 'CLOSED').length > 0;
    const hasFilteredData = filteredData.length >= 2;
    ...
    const minValue = hasFilteredData ? Math.min(...filteredData.map(d => d.value)) : 0;
    const maxValue = hasFilteredData ? Math.max(...filteredData.map(d => d.value)) : 0;
    ...
    const points = hasFilteredData ? filteredData.map((d, i) => `${getX(i)},${getY(d.value)}`).join(' ') : '';
    const areaPoints = hasFilteredData ? `${getX(0)},${height} ${points} ${getX(filteredData.length - 1)},${height}` : '';

    const latestPoint = hasFilteredData ? filteredData[filteredData.length - 1] : { value: 0 };
