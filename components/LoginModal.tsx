        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm sm:max-w-md overflow-hidden border border-slate-200 dark:border-slate-700 relative scale-100 transition-all">
                ...
                <div className="p-8 pt-10 space-y-6">
                    ...
                    {/* Permission Reminder (Always visible for clarity) */}
                    {!isForgotPassword && (
                        <div className="p-3 bg-amber-50 dark:bg-amber-900/10 rounded-lg border border-amber-100 dark:border-amber-900/30 flex gap-2 items-start animate-in fade-in duration-500">
                            <AlertCircle className="w-4 h-4 text-amber-600 dark:text-amber-500 mt-0.5 shrink-0" />
                            <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                                學員請使用與 <strong>Libao財經學院</strong> 一致的 E-mail，以便後台開通您的權限。
                            </p>
                        </div>
                    )}

                    {/* Google Login & Email Toggle (Initial View) */}
                    {!isForgotPassword && !showEmailForm && !isRegister && (
                        <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
                            ...
                            <button
                                ...
                                className="w-full flex items-center justify-center gap-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 dark:bg-slate-800/50 dark:border-slate-700 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 font-medium py-3 px-4 rounded-xl transition-all active:scale-[0.98]"
                            >
                                <Mail className="w-5 h-5" />
                                <span>使用其他 Email 登入</span>
                            </button>
                        </div>
                    )}

                    {/* Email/Password Form (Expanded View) */}
                    {(isForgotPassword || showEmailForm || isRegister) && (
                        <form onSubmit={handleSubmit} className="space-y-4 animate-in slide-in-from-bottom-2 duration-300">
                            {/* Google Login (Mini Version when form is open) */}
                            {!isForgotPassword && (
                                <button
                                    type="button"
                                    onClick={handleGoogle}
                                    disabled={loading}
                                    className="w-full mb-6 flex items-center justify-center gap-3 bg-white hover:bg-slate-50 border border-slate-200 dark:bg-slate-900 dark:border-slate-800 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-medium py-2.5 px-4 rounded-xl transition-all"
                                >
                        ...
                        <button
                            ...
                            onClick={() => {
                                if (isForgotPassword) {
                                    ...
                                    if (!isRegister) setShowEmailForm(true); // Auto expand if switching to register
                                }
                                setErrorMSG('');
                                setSuccessMSG('');
                            }}
