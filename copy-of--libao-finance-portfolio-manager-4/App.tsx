...
const App: React.FC = () => {
  ...
  const [portfolio, setPortfolio] = useState<PortfolioState>(() => {
    ...
    return {
      ...
      transactions: [],
      capitalLogs: [], // Start empty for new users
  ...
  useEffect(() => {
    const isOnboardingCompleted = localStorage.getItem('libao-onboarding-v1');
    const isLegacyUser = localStorage.getItem('libao-portfolio'); 

    if (!isOnboardingCompleted && !isLegacyUser) {
       // Only show onboarding for completely new users
       setShowOnboarding(true);
    } else {
       // Check tutorial for existing users
  ...
  const handleOnboardingComplete = (capital: number, categories: PositionCategory[]) => {
      ...
      // 2. Force Reload Page to clean state and remove modal
  ...
  const handleLogout = async () => {
     ...
     localStorage.removeItem('libao-tutorial-seen-v3');
     setShowOnboarding(true);
