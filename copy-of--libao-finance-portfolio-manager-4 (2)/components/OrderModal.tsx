...
import { X, Calculator, ArrowRight, RefreshCw, AlertCircle, Search, TrendingUp, Wallet, PieChart, Layers } from 'lucide-react';
...
const OrderModal: React.FC<OrderModalProps> = ({
  ...
  // --- Fee Settings State (Local to Modal, persisted) ---
  const [enableFees, setEnableFees] = useState<boolean>(() => {
    const saved = localStorage.getItem('libao_order_enable_fees');
    return saved !== null ? saved === 'true' : true;
  });

  const [usBroker, setUsBroker] = useState<'Firstrade' | 'IBKR' | 'Sub-brokerage'>(() => {
    return (localStorage.getItem('libao_us_broker') as 'Firstrade' | 'IBKR' | 'Sub-brokerage') || 'Firstrade';
  });

  // Sub-brokerage specific settings
  ...
  const [feeDiscount, setFeeDiscount] = useState<string>(() => {
    return localStorage.getItem('libao_tw_discount') || '6';
  });

  // Persist settings
  useEffect(() => {
    localStorage.setItem('libao_order_enable_fees', String(enableFees));
  }, [enableFees]);

  useEffect(() => {
    if (market === 'US') {
      localStorage.setItem('libao_us_broker', usBroker);
      localStorage.setItem('libao_us_sub_rate', subBrokerageRate);
      localStorage.setItem('libao_us_sub_min', subBrokerageMin);
    }
  }, [usBroker, subBrokerageRate, subBrokerageMin, market]);

  useEffect(() => {
    if (market === 'TW') {
      localStorage.setItem('libao_tw_discount', feeDiscount);
    }
  }, [feeDiscount, market]);
  ...
  const handleAutoCalcUS = useCallback(() => {
    ...
    // Even for sub-brokerage, SEC/TAF are pass-through usually, keeping logic standard
          ...
          {calcMode === 'SHARES' ? (
            <div>
              <div className="flex justify-between items-center mb-1">
                 ...
                 {isBuy && price && !isNaN(parseFloat(price)) && (
                    <button 
            ...
            {breakEvenPrice > 0 && currentSharesVal > 0 && (
               <div className="flex justify-between items-center mt-2 pt-2 border-t border-gray-200/50">
                  <span className="text-xs text-gray-500 font-bold">
                     {isBuy ? '損益兩平價 (Break-even)' : '實質成交均價 (Net Price)'}
                  </span>
