...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Check Tutorial Seen (Key updated to v3 to force display for all users)
  ...
  const handleConfirmDividends = (dividends: ScannedDividend[]) => {
    ...
    // executeTransaction handles individual toasts, but for bulk we might want one summary
    if (count > 0) showToast(`已確認 ${count} 筆股息入帳`, 'success');
  };
    ...
    try {
      ...
      portfolio.categories.forEach(cat => {
         cat.assets.forEach(a => {
            allAssets.push({ id: a.id, symbol: a.symbol, name: a.name, categoryName: cat.name, market: cat.market });
         });
      });
      
      for (const asset of allAssets) {
         const dividends = await stockService.getDividends(asset.symbol, asset.market);
      ...
      setScannedDividends(results);
       ...
       showToast("掃描過程中發生錯誤，請檢查網路連線。", 'error');
    } finally {
