...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (isAutoRefresh) {
      // Execute immediately (using the ref to be safe, though direct call is also fine here)
      latestUpdatePricesRef.current(true);
      
      autoRefreshIntervalRef.current = setInterval(() => {
        // Use the ref to ensure we call the latest function with the latest state
        if (!isUpdatingPrices) {
           latestUpdatePricesRef.current(true);
        }
      }, 60000);
  ...
  }, [isAutoRefresh]); // Only re-run if toggle changes
