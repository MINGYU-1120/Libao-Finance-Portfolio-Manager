...
const OrderModal: React.FC<OrderModalProps> = ({
  ...
  const runHybridSearch = useCallback((query: string) => {
      ...
      if (query.length >= 2) {
          ...
          searchTimeoutRef.current = setTimeout(async () => {
              try {
                  const onlineResults = await searchStocksYahoo(query, market);
                  setSuggestions(prev => {
  ...
  const handleSubmit = () => {
    ...
    // Fee/Tax inputs are assumed to be in the currency of the market display
    // So if market is US, input is USD, need to convert to TWD for storage.
